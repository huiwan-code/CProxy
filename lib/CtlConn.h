...
class CtlConn : public Conn,  public std::enable_shared_from_this<CtlConn>  {
  public:
    ...
    CtlConn(int fd, SP_EventLoop loop): Conn(fd, loop), out_buffer_(new Buffer(1024, 65536)), mutex_() {
    ...
    void setNotifyClientNeedProxyHandler(MsgHandler handler) {notifyClientNeedProxyHandler_ = handler;}
    void setNotifyProxyShutdownPeerConnHandler_(MsgHandler handler) {notifyProxyShutdownPeerConnHandler_ = handler;}
